...
export const placeOrder = async (userId: mongoose.Types.ObjectId, input: TPlaceOrderValidator) => {
  ...
  return withTransaction(async (session) => {
    ...
    if (payment_method === PaymentMethod.BANK_TRANSFER) {
      ...
      const returnUrl = input.return_url || `${APP_ORIGIN}/success?code=${order.code}`;
      const cancelUrl = input.cancel_url || `${APP_ORIGIN}/failed?reason=cancel&orderCode=${numericOrderCode}`;
